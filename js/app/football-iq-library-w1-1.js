...
function renderLibrary(){
  ...
  APP.innerHTML = `<section class="screen app active fiq-library-shell" data-football-iq-library>
    <header class="fiq-library-topbar"><button type="button" class="fiq-library-back" data-fiq-library-home aria-label="Back to Home">←</button><div><span>PitchIQ Academy</span><strong>Football IQ Training</strong></div><span class="fiq-library-level">LEVEL ${progress.level}</span></header>
    <main class="fiq-library-content"><section class="fiq-library-hero"><span class="fiq-library-kicker">Mission Library</span><h1>Train your<br><em>football brain.</em></h1><p>Build the habits that help you see more, decide earlier and play faster.</p></section>
      <nav class="fiq-library-tabs" aria-label="Mission library filters">${["recommended","browse","completed","locked"].map(tab=>`<button class="${activeTab===tab?"active":""}" type="button" data-fiq-tab="${tab}">${tab.charAt(0).toUpperCase()+tab.slice(1)}</button>`).join("")}</nav>
      <section class="fiq-library-panel" data-fiq-panel></section></main>
  </section>`;
  renderMissionList();
  return true;
}
function renderModule(){
  ...
  APP.innerHTML = `<section class="screen app active fiq-library-shell fiq-module-shell" data-football-iq-module="${module.id}">
    <header class="fiq-library-topbar"><button type="button" class="fiq-library-back" data-fiq-module-back aria-label="Back to Mission Library">←</button><div><span>PitchIQ Academy</span><strong>Football IQ Training</strong></div><span class="fiq-library-level">${progress.mastery}</span></header>
    <main class="fiq-library-content"><section class="fiq-module-hero"><span class="fiq-module-icon">${module.icon}</span><span class="fiq-library-kicker">Football IQ Module</span><h1>${module.title}</h1><p>${module.description}</p><strong>${module.coachingPrompt}</strong></section>
      <section class="fiq-module-progress" aria-label="Module progress"><div class="fiq-module-progress-head"><div><small>Mastery score</small><strong>${progress.percent}%</strong></div><span>${progress.completed} of ${progress.total} complete</span></div><div class="fiq-module-progress-track"><i style="width:${progress.percent}%"></i></div><div class="fiq-module-progress-meta"><span>${progress.mastery}</span><span>${progress.totalMinutes} min available</span><span>${progress.lastTrained ? `Last trained ${formatFootballIqDate(progress.lastTrained)}` : "Ready to begin"}</span></div></section>
      ...
      <section class="fiq-module-list"><div class="fiq-library-section-head"><span>Missions</span><small>${missions.length} total</small></div>${missions.map(moduleMissionRow).join("") || `<div class="fiq-mission-empty"><strong>Missions coming soon</strong><p>This module is ready for its first training mission.</p></div>`}</section></main>
  </section>`;
  return true;
}
function progressMarkup(mission){
  return `<div class="fiq-detail-progress"><div><small>Previous best</small><strong>${mission.completed ? `${mission.personalBest}%` : "First attempt"}</strong></div><div><small>Attempts</small><strong>${mission.attempts || "None yet"}</strong></div><div><small>Completed</small><strong>${mission.completed ? "Yes" : "Not yet"}</strong></div><div><small>Last played</small><strong>${mission.lastPlayed ? formatFootballIqDate(mission.lastPlayed) : "Not recorded"}</strong></div></div>`;
}
function renderMissionDetail(){
  ...
  APP.innerHTML = `<section class="screen app active fiq-detail-shell" data-football-iq-detail="${mission.id}">
    <header class="fiq-detail-topbar"><button type="button" class="fiq-detail-back" data-fiq-detail-back data-fiq-detail-category="${mission.category}" aria-label="Back to ${category}">←</button><div><span>PitchIQ Academy</span><strong>Football IQ Training</strong></div><span class="fiq-detail-level">LEVEL ${currentProgress().level}</span></header>
    <main class="fiq-detail-content"><section class="fiq-detail-hero"><span class="fiq-detail-category">${category}</span><h1>${mission.title}</h1><p>${mission.description}</p></section>
      <section class="fiq-detail-meta" aria-label="Mission information"><div class="fiq-detail-stat"><small>Estimated time</small><strong>${mission.minutes} min</strong></div><div class="fiq-detail-stat"><small>XP reward</small><strong>${mission.xp} XP</strong></div><div class="fiq-detail-stat"><small>Difficulty</small><strong>${stars(mission.difficulty)}</strong></div><div class="fiq-detail-stat"><small>Category</small><strong>${category}</strong></div></section>
      ...
      <section class="fiq-detail-panel fiq-detail-related"><h2>Next recommended</h2>${related.map(item=>missionCard(item,true)).join("") || `<p>No related missions available yet.</p>`}</section></main>
  </section>`;
  return true;
}
