...
const STYLE_HREF="css/home-adaptive-mission-h8.css?v=sprint-h12-coach-intelligence-20260721";
...
function ensureStylesheet(){ let link=document.getElementById(STYLE_ID); if(!link){ link=document.createElement("link");link.id=STYLE_ID;link.rel="stylesheet";document.head.appendChild(link); } if(link.getAttribute("href")!==STYLE_HREF) link.setAttribute("href",STYLE_HREF); }
function readSelection(storage=globalThis.localStorage){ try{ const value=JSON.parse(storage?.getItem?.(ADAPTIVE_CURRENT_KEY)||"null");return value?.mission?.id?value:null; }catch{return null;} }
function text(value,fallback=""){ return typeof value==="string"&&value.trim()?value.trim():fallback; }
function number(value,fallback=0){ const parsed=Number(value);return Number.isFinite(parsed)?parsed:fallback; }
function escapeHtml(value){ return String(value??"").replace(/[&<>'"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[c]); }
function isComplete(selection){ const state=text(selection?.status||selection?.state||selection?.mission?.status).toLowerCase();return selection?.completed===true||selection?.mission?.completed===true||["complete","completed","done"].includes(state); }
...
function activeMission(selection=readSelection()){
  ...
  return { id:mission.id,title:text(mission.title||mission.name,"Continue your mission"),world:text(mission.world||mission.domain||mission.categoryLabel,"Today's Training"),focus:text(mission.coachingFocus||mission.focus||mission.description||selection.reason,"Continue building the habit from your current mission."),minutes:number(mission.minutes||mission.durationMinutes||selection.minutes,8),xp:number(mission.xp||mission.xpReward||selection.xp,20),route:"training",source:"active",progress:number(selection.progress||mission.progress,0) };
}
