/* Direct landing-to-app router bridge. Keeps swipe ownership unchanged. */
...
async function leaveSplash(){
  if(routing||!landingVisible())return;
  routing=true;
  const target=onboardingComplete()?"home":"onboard";
  const url=new URL(window.location.href);
  url.searchParams.set("dev","1");
  url.searchParams.set("routerBridge","2");
  url.hash=target;
  history.replaceState(null,"",url.toString());
  const devRouteButton=document.querySelector(`[data-dev-route="${target}"]`);
  if(devRouteButton){ devRouteButton.click(); }
  if(!landingVisible())return;
  try{
    await import(`./main.js?router-bridge=${Date.now()}`);
  }catch(error){
    console.error("[PitchIQ landing router bridge] in-memory router restart failed",error);
  }
  if(!landingVisible())return;
