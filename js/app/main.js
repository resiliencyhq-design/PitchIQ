...
function onboardingComplete(){ return localStorage.getItem(ONBOARDING_COMPLETE_KEY) === "true" && !!localStorage.getItem(PLAYER_NAME_KEY) && !!localStorage.getItem(SELECTED_POSITION_KEY); }
function syncProfileFromOnboardingKeys(){ const name = localStorage.getItem(PLAYER_NAME_KEY); const position = localStorage.getItem(SELECTED_POSITION_KEY); if(name) state.profile.name = name; if(position) state.profile.position = position; }
...
function clearOnboardingLock(){ localStorage.removeItem(ONBOARDING_COMPLETE_KEY); localStorage.removeItem(PLAYER_NAME_KEY); localStorage.removeItem(SELECTED_POSITION_KEY); state.profile.name = ""; state.profile.position = "Winger"; state.profile.createdAt = null; saveState(state); }
...
function applyDeveloperBorder(){ document.body.classList.toggle("pitchiq-dev-border", DEV_MODE_ENABLED && devBorderEnabled); }
...
function routeLabel(route){ if(route === "splash") return "Splash"; if(route === "onboard") return "Create Player"; if(route === "home") return "Home"; if(route === "training") return "Training"; if(route === "results") return "Results"; if(route === "player") return "Player"; return route.charAt(0).toUpperCase()+route.slice(1); }

function renderDeveloperPanel(){
  if(!DEV_MODE_ENABLED){ document.body.classList.remove("pitchiq-dev-border"); return; }
  applyDeveloperBorder();
  let toggle = document.getElementById("pitchiq-dev-toggle");
  if(!toggle){ toggle = document.createElement("button"); toggle.id = "pitchiq-dev-toggle"; toggle.type = "button"; toggle.setAttribute("aria-label", "Toggle developer navigation"); toggle.style.cssText = "position:fixed;top:calc(env(safe-area-inset-top,0px) + 8px);left:calc(env(safe-area-inset-left,0px) + 8px);z-index:10000;width:44px;height:44px;min-height:44px;padding:0;border-radius:14px;background:rgba(10,16,28,.62);color:#d7ff2e;border:1px solid rgba(215,255,46,.22);font:700 18px/1 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;box-shadow:0 8px 24px rgba(0,0,0,.22);backdrop-filter:blur(18px)"; document.body.appendChild(toggle); toggle.addEventListener("click",()=>{ devPanelOpen = !devPanelOpen; sessionStorage.setItem("pitchiq-dev-open", devPanelOpen ? "1" : "0"); renderDeveloperPanel(); }); }
  toggle.textContent = devPanelOpen ? "×" : "☰";
  let panel = document.getElementById("pitchiq-dev-panel");
  if(!panel){ panel = document.createElement("aside"); panel.id = "pitchiq-dev-panel"; panel.setAttribute("aria-label", "PitchIQ Developer navigation"); panel.style.cssText = "position:fixed;top:calc(env(safe-area-inset-top,0px) + 58px);left:calc(env(safe-area-inset-left,0px) + 8px);z-index:9999;background:rgba(10,16,28,.88);color:#fff;border:1px solid rgba(255,255,255,.18);border-radius:12px;padding:10px;font:12px/1.3 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;box-shadow:0 8px 24px rgba(0,0,0,.25);max-width:190px;backdrop-filter:blur(18px)"; document.body.appendChild(panel); }
  ...
  panel.querySelectorAll("[data-dev-route]").forEach(button=>button.addEventListener("click",()=>{ const route = button.dataset.devRoute; console.log(`[PitchIQ Dev] Jump to ${route}`); goto(route); }));
  panel.querySelector("[data-dev-border-toggle]")?.addEventListener("click",()=>{ devBorderEnabled = !devBorderEnabled; localStorage.setItem("pitchiqDevBorderEnabled", devBorderEnabled ? "true" : "false"); applyDeveloperBorder(); renderDeveloperPanel(); });
  panel.querySelector("[data-dev-reset-onboarding]")?.addEventListener("click",()=>{ clearOnboardingLock(); selectedPosition = ""; toast("Onboarding reset"); goto("splash"); });
}
...
function trainingView(){ return { stage: trainingStage, selectedDrillId, selectedDrill: selectedDrill(), missionDrill: missionDrill(), difficulty: selectedDifficulty, summary: trainingSummary, time: training.time, score: training.score, combo: training.combo, cueDisplay: cue?.display, instruction: activeSession?.currentCue ? "Say or tap: " + activeSession.currentCue.acceptedResponses[0].toUpperCase() + " • " + selectedDifficulty.toUpperCase() : "Say or tap the cue.", rewardName: state.game?.dailyDone ? "Daily Academy Pack" : "Academy Training Pack" }; }
function renderTrainingRoute(){ render("training"); }
function setTrainingStage(stage){ trainingStage = stage; renderTrainingRoute(); }

function getOnboardName(){ return document.getElementById("nameInput")?.value?.trim() || ""; }
function showOnboardStep(step){ document.querySelectorAll("[data-onboard-step]").forEach(section=>{ section.hidden = section.dataset.onboardStep !== String(step); }); document.querySelectorAll("[data-step-dot]").forEach(dot=>{ dot.classList.toggle("active", dot.dataset.stepDot === String(step)); }); if(step === 3){ const name = getOnboardName(); const confirmName = document.getElementById("confirmName"); const confirmPosition = document.getElementById("confirmPosition"); if(confirmName) confirmName.textContent = name || "—"; if(confirmPosition) confirmPosition.textContent = selectedPosition || "—"; } }
function onboardNextName(){ const name = getOnboardName(); if(!name) return toast("Enter player name"); showOnboardStep(2); }
function onboardNextPosition(){ if(!selectedPosition) return toast("Choose your position"); showOnboardStep(3); }
function bindOnboardWizard(){
  const nameInput = document.getElementById("nameInput");
  const nameCta = document.querySelector('[data-action="onboard-next-name"]');
  const positionCta = document.querySelector('[data-action="onboard-next-position"]');
  ...
  selectedPosition = "";
  nameCta?.setAttribute("disabled", "true");
  positionCta?.setAttribute("disabled", "true");

  const updateNameCta = ()=>{ if(getOnboardName()) nameCta?.removeAttribute("disabled"); else nameCta?.setAttribute("disabled", "true"); };
  const updatePositionCta = ()=>{ if(selectedPosition) positionCta?.removeAttribute("disabled"); else positionCta?.setAttribute("disabled", "true"); };
  const setPosition = (node)=>{ if(!node) return; nodes.forEach(n=>n.classList.remove("selected","targeted")); node.classList.add("selected"); selectedPosition = node.dataset.pos; if(ball){ ball.dataset.pos = selectedPosition; ball.style.gridArea = node.dataset.slot || node.style.gridArea; } if(confirm) confirm.innerHTML = `<span>Selected:</span><b>${selectedPosition}</b>`; updatePositionCta(); };

  nameInput.addEventListener("input", updateNameCta);
  if(pitch && ball && nodes.length){
    const nearestNode = (clientX, clientY)=>{ let nearest = null; let best = Infinity; nodes.forEach(node=>{ const r = node.getBoundingClientRect(); const x = r.left + r.width/2; const y = r.top + r.height/2; const d = Math.hypot(clientX - x, clientY - y); if(d < best){ best = d; nearest = node; } }); return nearest; };
...
function finishTraining(){ if(training.timer) clearInterval(training.timer); training.timer = null; completeDaily(state); if(activeSession){ const attempts = activeSession.results.length; const correctCount = activeSession.results.filter(r=>r.correct).length; const accuracy = attempts ? Math.round(correctCount / attempts * 100) : 0; trainingSummary = { attempts, correct:correctCount, accuracy, score:training.score, xp:state.game.lastXp, combo:state.game.bestCombo }; state.analytics.sessions.push({ id:activeSession.id, drill:activeSession.drill.id, difficulty:selectedDifficulty, score:training.score, results:activeSession.results, endedAt:Date.now() }); } else { trainingSummary = { attempts:0, correct:0, accuracy:0, score:training.score, xp:state.game.lastXp, combo:state.game.bestCombo }; } trainingStage = "results"; toast("Session complete 🏆"); goto("results"); }
function startVoice(){ if(trainingStage !== "live") return toast("Start a live session first"); voice = new PitchIQVoiceEngine({ onStatus:(s)=>toast("Voice: "+s), onTranscript:(t)=>{}, onResult:(r)=>{ if(r.correct){ const leveled = addXP(state, r.xpAwarded); if(activeSession) activeSession.results.push({ cueId:r.cueId, correct:true, xpAwarded:r.xpAwarded, reactionMs:null, detected:false, timestamp:r.timestamp }); toast(leveled ? "LEVEL UP 🏆" : "+"+r.xpAwarded+" XP 🎤"); nextCue(); } else toast("Heard: "+r.transcript); } }); if(!voice.start(cue)) toast("Voice not supported"); }
async function testVideo(){ try{ const st = await navigator.mediaDevices.getUserMedia({video:true,audio:false}); st.getTracks().forEach(t=>t.stop()); document.getElementById("videoStatus").textContent="Video works"; toast("Camera test passed"); } catch{ document.getElementById("videoStatus").textContent="Camera blocked"; toast("Camera failed"); } }
async function startCamera(mode){ const video = document.getElementById("video"), canvas = document.getElementById("motion"), cueEl = document.getElementById("cameraCue"); camera = new PitchIQCameraEngine(video, canvas, { onStatus:(s)=>{ const st=document.getElementById("cameraStatus"); if(st) st.textContent=s; if(cueEl && s==="missed"){ cueEl.textContent="MISS"; cueEl.className="camera-cue miss"; } }, onMotionScore:(m)=>{ const mr=document.getElementById("motionRead"); if(mr) mr.textContent="Motion "+Math.round(m); }, onResult:(r)=>{ if(r.detected){ cueEl.textContent = r.reactionMs+" ms"; cueEl.className = "camera-cue detected"; document.getElementById("lastRT").textContent = r.reactionMs+" ms"; camScore += r.xpAwarded; document.getElementById("camScore").textContent = camScore; if(!state.analytics.bestReaction || r.reactionMs < state.analytics.bestReaction){ state.analytics.bestReaction = r.reactionMs; document.getElementById("camBest").textContent = r.reactionMs+" ms"; } const leveled = addXP(state, r.xpAwarded); toast(leveled ? "LEVEL UP 🏆" : "+"+r.xpAwarded+" XP"); } } }); const ok = await camera.start(mode); toast(ok ? "Camera started" : "Camera unavailable"); }
...
function openPackAction(){ if(!state.game.dailyDone) return toast("Complete today's mission first"); const reward = openReward(state, REWARDS); document.getElementById("pack")?.classList.add("open"); document.getElementById("rewardTitle").textContent = reward.name; document.getElementById("rewardText").textContent = "Unlocked " + reward.tier + " reward"; toast("Unlocked: "+reward.name); saveState(state); }
try { render(devInitialRoute()); window.__PITCHIQ_READY__ = true; } catch (error) { showRenderError(error, currentRoute || "startup"); }
