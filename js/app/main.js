...
function applyDeveloperBorder(){
  document.body.classList.toggle("pitchiq-dev-border", DEV_MODE_ENABLED && devBorderEnabled);
}
...
function routeLabel(route){
  ...
  if(route === "onboard") return "Create Player";
...
function renderDeveloperPanel(){
  ...
  panel.querySelector("[data-dev-border-toggle]")?.addEventListener("click",()=>{
    devBorderEnabled = !devBorderEnabled;
    localStorage.setItem("pitchiqDevBorderEnabled", devBorderEnabled ? "true" : "false");
    applyDeveloperBorder();
    renderDeveloperPanel();
  });
...
function bindScreen(){
  document.querySelectorAll("[data-route]").forEach(el=>el.addEventListener("click",()=>{
    ...
    if (!VALID_ROUTES.has(route)) { console.warn("[PitchIQ route] Route missing:", route); toast("Route not available yet"); return; }
    goto(route);
  }));
  ...
  document.querySelectorAll("[data-pos]:not(.pitch-node)").forEach(btn=>btn.addEventListener("click",()=>{ document.querySelectorAll("[data-pos]").forEach(b=>b.classList.remove("selected")); btn.classList.add("selected"); selectedPosition = btn.dataset.pos; }));
  document.querySelectorAll("[data-drill]").forEach(btn=>btn.addEventListener("click",()=>{ document.querySelectorAll("[data-drill]").forEach(b=>b.classList.remove("active")); btn.classList.add("active"); selectedDrillId = btn.dataset.drill; toast("Selected: "+btn.querySelector("b").textContent); }));
