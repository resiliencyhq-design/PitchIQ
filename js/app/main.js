...
function syncPlayerFromService(){ const player=PlayerService.getPlayer(); state.profile={...(state.profile||{}),...player}; return player; }
function onboardingComplete(){ const player=syncPlayerFromService(); return Boolean(player.name&&player.position); }
function completeOnboarding(name,position){ const player=PlayerService.completeOnboarding({name,position}); state.profile={...(state.profile||{}),...player}; }
function clearOnboardingLock(){ PlayerService.resetPlayer(); state.profile={name:"",number:"1",position:"",style:"Creator",avatar:"default",goal:"React faster",createdAt:null}; saveState(state); }
