...
function readSelection() {
  try { return JSON.parse(localStorage.getItem(ADAPTIVE_CURRENT_KEY) || "null"); }
  catch { return null; }
}

function readHistory() {
  try {
    const value = JSON.parse(localStorage.getItem(REFLECTION_HISTORY_KEY) || "[]");
...
export function buildPostTrainingReflection(selection = readSelection()) {
  ...
  return Object.freeze({
    ...
    missionTitle: mission.title || "today’s mission",
    prompt: `During ${mission.title || "that mission"}, how often did you notice the coaching cue?`,
    options: OPTIONS
  });
}
...
export function saveCoachReflection({ selection = readSelection(), responseId = "skipped" } = {}) {
  ...
  try {
    localStorage.setItem(REFLECTION_CURRENT_KEY, JSON.stringify(record));
    localStorage.setItem(REFLECTION_HISTORY_KEY, JSON.stringify([...readHistory(), record].slice(-MAX_HISTORY)));
    window.dispatchEvent(new CustomEvent("pitchiq:coach-reflection", { detail: record }));
  } catch (error) {
    console.warn("[PitchIQ Coach] Reflection could not be stored", error);
  }
  return record;
}

function closeReflection() {
  activeReflection?.remove();
...
export function showPostTrainingCoachReflection(selection = readSelection()) {
  ...
  overlay.innerHTML = `
    <section class="post-training-coach-reflection__card">
      <p class="post-training-coach-reflection__coach">${model.coachName}</p>
      <p class="post-training-coach-reflection__role">${model.coachRole}</p>
      <p class="post-training-coach-reflection__kicker">Quick reflection</p>
      <h2 id="postTrainingCoachReflectionTitle">${model.missionTitle}</h2>
      <p class="post-training-coach-reflection__prompt">${model.prompt}</p>
      <div class="post-training-coach-reflection__options">
        ${model.options.map(option => `<button type="button" data-reflection-response="${option.id}">${option.label}</button>`).join("")}
      </div>
      <p class="post-training-coach-reflection__response" data-reflection-coach-response hidden></p>
      <button class="post-training-coach-reflection__continue" type="button" data-reflection-continue hidden>Continue →</button>
      <button class="post-training-coach-reflection__skip" type="button" data-reflection-skip>Skip reflection</button>
      <small>This reflection supports coaching context only. It does not change Football IQ scores or XP.</small>
    </section>`;
  ...
  overlay.querySelectorAll("[data-reflection-response]").forEach(button => {
  ...
  overlay.querySelector("[data-reflection-skip]")?.addEventListener("click", () => {
    saveCoachReflection({ selection, responseId: "skipped" });
    closeReflection();
  }, { once: true });
  document.body.appendChild(overlay);
  activeReflection = overlay;
  overlay.querySelector("[data-reflection-response]")?.focus();
  return overlay;
}
...
if (typeof document !== "undefined") {
  document.addEventListener("click", event => {
    ...
    if (action === "finish-live-session") setTimeout(() => showPostTrainingCoachReflection(), 0);
  }, true);
}
